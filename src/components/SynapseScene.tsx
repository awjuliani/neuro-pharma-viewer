...
const ligandTooltipCopy = {
  ...
  antagonist: {
    body: "A drug ligand that occupies an orthosteric receptor site without activating it.",
    title: "Antagonist molecule"
  },
  pam: {
    body: "A positive allosteric modulator that binds a regulatory site and boosts later transmitter-driven activation.",
    title: "PAM molecule"
  },
  releaser: {
    body: "A drug ligand that binds transporter sites and can drive transporter-mediated transmitter efflux.",
    title: "Releaser molecule"
  },
  reuptake_inhibitor: {
    body: "A drug ligand that binds transporter sites and blocks transmitter reuptake.",
    title: "Reuptake inhibitor"
  },
  transmitter: {
    body: "Endogenous transmitter diffusing through the cleft. It can bind an open receptor, be cleared by local reuptake transporters, or diffuse out of the modeled cleft.",
    title: "Transmitter"
  }
...
const dockedLigandTooltipCopy = (ligand: DockedLigand) => {
  if (ligand.target.kind === "transporter") {
    return ligand.ligandKind === "releaser"
      ? {
          body: "This transporter is in a reversed state and driving transmitter efflux into the cleft.",
          title: "Reversed transporter"
        }
      : {
          body: "This occupied transporter is blocked from clearing transmitter by reuptake.",
  ...
  if (ligand.ligandKind === "pam") {
    return {
      body: "A PAM is docked at an allosteric regulatory site. It does not signal alone.",
      title: "Docked PAM"
    };
  }
  ...
  if (ligand.ligandKind === "agonist") {
    return {
      body: "The agonist is docked at an orthosteric receptor site and directly drives receptor signaling.",
  ...
  if (ligand.ligandKind === "antagonist") {
    return {
      body: "The antagonist is occupying the orthosteric receptor site and preventing activation here.",
      title: "Docked antagonist"
    };
  }

  return {
    body: "The transmitter is bound at the orthosteric receptor site for a brief activation window.",
...
const makeTooltip = (
  ...
  if (vesicle) {
    return {
      ...position,
      body: "A synaptic vesicle drifting toward the presynaptic membrane. Transmitter appears outside only after fusion.",
  ...
  if (allostericSite) {
    return {
      ...position,
      body: "An allosteric regulatory site. PAM molecules can bind here without signaling alone, then boost later transmitter-driven activation.",
  ...
  if (transporter) {
    ...
    return {
      ...
      body: occupancy.leaking
        ? "This transporter is reversed and driving transmitter efflux into the cleft."
        : occupancy.ligand
          ? "This transporter is occupied by a drug molecule and cannot reuptake transmitter."
          : occupancy.absorbing
            ? "This transporter is actively clearing transmitter by reuptake into the axon."
            : "An open reuptake transporter that can clear nearby transmitter from the cleft.",
      title: occupancy.leaking ? "Reversed transporter" : "Reuptake transporter"
    };
  }
  ...
  if (isInsideDendrite(sceneX, sceneY)) {
    return {
      ...position,
      body: "Postsynaptic side. Receptor activations here produce the visual and audible signal events.",
      title: "Dendrite"
    };
  }
...
function PostsynapticSignalTimeline({
  ...
  return (
    <div className="note-timeline">
      <svg
        aria-label="Postsynaptic signal timeline"
